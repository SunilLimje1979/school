    ...
    if request.method == "GET":
        ...
        if response.status_code == 200:
            ...
            if response.json().get('response') == True:
                ...
                # If the response is not true, return an error response
                messages.success(request, "Mobile number not found")
                return redirect('login')  # Assuming 'login' is the name of your login view
        else:
            # If the first API request was unsuccessful, return an error response
            return HttpResponse("Error: Unable to fetch data from the API")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for student in matching_students:
                ...
                temp_file = code128.save('temp_barcode')
